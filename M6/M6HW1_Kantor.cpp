...
int main()
{
    ...
    // ----- NPC parallel arrays: one name/hint per room (empty = no NPC) -----
    string npcNames[NUM_ROOMS];
    string npcHints[NUM_ROOMS];
    for (int r = 0; r < NUM_ROOMS; r++)
    {
        npcNames[r] = "";
        npcHints[r] = "";
    }
    npcNames[SOLAR] = "Dr. Vasquez";
    npcHints[SOLAR] = "Dr. Vasquez is wedged between two solar array struts, visor fogged.\n"
                      "She grabs your arm: \"I've been stuck here since the breach. Listen —\n"
                      "I saw a key card on the rack in the Equipment Armory, just east of here.\n"
                      "Couldn't reach it myself. You need it to get into the Bubble.\"";
    ...
    cout << "Commands: north/n, east/e, south/s, west/w, look, talk, quit" << endl;
    ...
    while (true)
    {
        ...
        if (command == "talk")
        {
            if (!npcNames[currentRoom].empty())
                cout << "\n" << npcHints[currentRoom] << endl;
            else
